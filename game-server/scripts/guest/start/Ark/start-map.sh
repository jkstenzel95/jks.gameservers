...
ShooterGame/Binaries/Linux/ShooterGameServer "${MAP_NAME}?listen?Multihome=0.0.0.0?SessionName=${SessionName}?MaxPlayers=${MAX_PLAYERS}?QueryPort=${QUERY_PORT}?RCONPort=${RCON_PORT}?Port=${PORT}?ServerAdminPassword=${SERVER_ADMIN_PASSWORD}?AltSaveDirectoryName=./MapSaves/${MAP_NAME}" -server -log -NoTransferFromFiltering -clusterid="${CLUSTER_ID}"