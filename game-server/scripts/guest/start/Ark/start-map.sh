...
ShooterGame/Binaries/Linux/ShooterGameServer "${MAP_CODE}?listen?Multihome=0.0.0.0?SessionName=${SessionName}?MaxPlayers=${MAX_PLAYERS}?QueryPort=${QUERY_PORT}?RCONPort=${RCON_PORT}?Port=${PORT}?ServerAdminPassword=${SERVER_ADMIN_PASSWORD}?AltSaveDirectoryName=${MAP_NAME}?OverrideOfficialDifficulty=5.0?GameModIds=${MOD_LIST}" -server -log -NoTransferFromFiltering -clusterid="${CLUSTER_ID}"
ShooterGame/Binaries/Linux/ShooterGameServer "${MAP_CODE}?listen?Multihome=0.0.0.0?SessionName=${SessionName}?MaxPlayers=${MAX_PLAYERS}?QueryPort=${QUERY_PORT}?RCONPort=${RCON_PORT}?Port=${PORT}?ServerAdminPassword=${SERVER_ADMIN_PASSWORD}?AltSaveDirectoryName=TheCenter?OverrideOfficialDifficulty=5.0?GameModIds=684970590" -server -log -NoTransferFromFiltering -exclusivejoin -clusterid="jkstest"